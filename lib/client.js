...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var BLUE_FILL = "rgba(90,140,255,.10)";
    ...
    function rowStyle(level, active) {
      return {
        display: "flex",
        alignItems: "center",
        gap: 8,
        ...
        lineHeight: "18px",
        color: active ? BLUE : (level === 0 ? C.labelSecondary : C.labelTertiary),
        fontWeight: active ? 600 : (level === 0 ? 500 : 400),
        background: active ? BLUE_FILL : "transparent"
      };
    }

    function barStyle(active, kind) {
      return {
        flex: "none",
      ...
      return el("div", { className: "dsh-toc-panel", style: style },
        ...
        el("div", {
          ...
          topics.map(function (topic) {
            ...
            var active = activeKey !== null && lastPerNode[topic.nodeKey] === topic.key;
            return el("div", {
              key: topic.key,
              "data-dsh-toc-row": "",
              role: "button",
              ...
              style: rowStyle(topic.level, active)
            },
              el("span", {
                style: {
              ...
              el("span", { "data-kind": topic.kind, style: barStyle(active, topic.kind) })
            );
          })
        )
      );
    ...
    function TocFeature(props) {
      ...
      useEffect(function () {
        ...
        var computeActive = function () {
          ...
          // 滚动到底部时高亮最后一个主题
          if (sp.scrollHeight - sp.scrollTop - sp.clientHeight <= 24) {
            var last = rows[rows.length - 1];
            if (last) active = last.dataset.chatAnchorKey;
          }
